...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "hierarchy") {
      ...
      const cacheKey = `meta:hierarchy5:${preset}:${t}`;   // created/updated 추가로 키 갱신 (2026-08-27)
      const hit = await cacheGet(cacheKey, 60 * 1000);
      if (hit) return json(hit);
      ...
      const ensureCamp = (id: string, name = "") => {
        if (!cMap.has(id)) cMap.set(id, { id, name, status: "", budget: 0, created: "", updated: "", spend: 0, purchases: 0, value: 0, clicks: 0, adsets: new Map() });
        return cMap.get(id)!;
      };
