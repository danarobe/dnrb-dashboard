...
- **괄호 표기 차이 폴백 `stkCoreOpt`(2026-09-09 실사례)**: 베일 블라우스가 안정재고 편성 위 표에서 '결제 0건→줄이기', 아래 '신규 편성'에 동시에 뜬 원인 = 셀메이트 옵션 `S(55~66)/M(77~88)` vs 카페24 `S(55~77)/M(77반~)` — 사이즈 범위 표기만 달라 정확·느슨 매칭 모두 실패. 각 값에서 괄호·대괄호 내용을 지운 토큰(`stkCoreTokens`)이 같으면 같은 옵션으로 보되, **양쪽 다 괄호가 없으면 false, 같은 상품 안에서 괄호 뺀 옵션이 유일하고 후보도 1개일 때만 채택**(FREE(55~66)/FREE(77~88)처럼 괄호로만 구분되는 옵션 보호). 안정재고 편성(`coreCount`)·자체제작 주문 점검(셀메이트·이지픽, `coreUnique`)·npm MadeCheckClient 3곳 동일. 근본 해결은 셀메이트 옵션명을 카페24와 맞추는 것.
