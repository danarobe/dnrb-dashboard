...
## 0-1. 이어서 할 때 (2026-09-22 세션 인수인계 — 새 컨텍스트는 여기부터)

**먼저 할 일**: 이 블록 → §1 작업 관례 → 필요한 메뉴의 본문 섹션 순으로 읽는다. 사용자는 비개발자(쉬운 한국어, 결과 중심). 사용자 본인 워크스페이스 계정은 아이디 `admin`(§1 참고) — "나만" 요청은 이 계정 기준.

**이번 세션(2026-09-09~22)에 만든 것 — 상세는 각 섹션에 있음**
- 자체제작 주문 점검(#madechk): 이지픽 엑셀 = 입고 예정, 매칭불가 + 수동 매칭(`made_products.stk_name`), **소메뉴 ① 재고·입고 점검(맨 위) ② 주문 점검**, 공용 파일 저장(`made_check_files`), 지정 상품(`made_watch_products`), 부족 = 현재 재고 기준·주의 배지(5개 미만/안정재고 미달) → §5 자체제작.
- 근무 관리(#wm): 식대 규칙(9/1 13:00 · 8/1부터 출근 하한 없음), 명세서 파일 업로드(`wm_payslips`, 비공개 버킷 `wm-payslips`) + 마이페이지 본인 열람, 휴가 관리 직원별 보기, **출장 여비**(제출·사비 지출·영수증 `wm-receipts`·관리자 확인/보완/재제출·상세 모달·초과근로 엑셀, `wm_trip_claims/receipts`), **서류 요청**(재직증명서 PDF 저장 `docSavePdf`·출력 요청 `wm_doc_requests`·관리자 인쇄/출력 완료), 직원 구매요청 담당자/주문자 분리(`orderer_name`) → §7-5.
...
**진행 중 / 사용자 답 대기**
1. **반품 스캔 — 네이버페이센터 반품/교환 엑셀 샘플 대기**: 실제 열 이름을 못 봤다. (2026-09-22b~d: 문서 암호·같은 송장 합치기·기간 선택·바로가기 버튼까지 추가됨 — §7-0. **카페24 주문 상세 바로가기 주소 형식 실검증 대기**: 사용자가 눌러 보고 틀리면 실제 주소를 받아 기본값 수정. 네이버페이센터 상태 딥링크는 불가로 확정(수취인명 복사로 대체). 사용자 브라우저는 **엣지**(크롬 확장 사용 불가).) 받으면 `rscanNaverUpload`의 헤더 후보(수거송장번호·상품주문번호·클레임구분·사유·택배사·요청일)를 실양식에 맞추고 실파일로 검증. 경고 코드 기본값(K·V·D / C·J·W)은 6개월 실데이터 추정 — 사용자 실사용 피드백 반영.
2. **재고·입고 점검 — 사용자가 실제 파일을 아직 안 올림**(테스트 업로드는 삭제). 셀메이트 CSV가 '자체제작'만 담겨 'made' 상품 8건이 '매칭 안 됨'으로 나온 점 안내함 → 다음 CSV는 made 포함/전체로. 상품관리 시스템(npm)에도 같은 소메뉴를 넣을지 미정(사용자 답 대기).
3. 친구 광고 대시보드: 친구가 모바일 최적화 완료 통보 → 우리 쪽 폰 폭 검수는 아직 안 함(사용자가 원하면 임시 계정으로 확인). 공동 작업자 초대는 보류.
...
- 로컬 프리뷰 서버(preview_start dnrb-dashboard)는 자주 죽는다 — navigate 실패/빈 페이지면 preview_start 다시. 브라우저 페인이 숨겨져 있으면 스크린샷 대신 javascript_tool로 innerText 검증.
- 배포 확인: Pages는 `curl … | grep -c '고유 문자열'` 폴링(2~4분), Vercel(npm)은 gh api deployments 폴링.
- QA 계정은 끝나면 반드시 삭제(app_users qa_%, wm_employees QA%, 관련 행). 실제 직원 데이터(신청서 등)는 건드리지 않는다.
- 카페24 access_token은 만료가 잦다(2시간) — 직접 API를 칠 땐 임시 QA 관리자 토큰으로 함수 하나 호출해 갱신시킨 뒤 api_tokens에서 읽는다.
...
- **사용자(대표) 본인 워크스페이스 계정 = 아이디 `admin`(이름 조범준). 박민규 `pmk8668us`·김다나 `rlaeksk1994`는 다른 관리자.** "나만" 같은 요청은 `admin` 기준 (2026-09-13 확인).

- index.html 대량 수정: python 치환 스크립트를 **파일로 저장**해 실행, 앵커는 `cat -vet`로 빈 줄까지 확인, 수정 후 `<script>` 블록 `new Function` 문법 검사 필수 (2026-09-22).
...
- **광고↔상품 매칭·활성 목록 보강(2026-09-23)**: ① **카페24 애널리틱스 통계(summary)의 product_name**이 9/20~ 일부 상품에 ' - Danarobe' 접미사를 붙여 내려옴(관리자 API·관리자 화면 상품명에는 없음 — 카페24 통계 쪽 표기 변화)으로 `paKey` 핵심명이 깨져 판매 성과 'ON 광고'·에이전트가 광고 0개로 오판 → `PA_BRAND_SUFFIX`로 먼저 제거(에이전트 def.ts와 동일). ② meta-ads `activeads`: ACTIVE 필터 목록(130)에 오늘 지출 광고 83개가 빠짐(실측, 9/14~17 _test 소재) → 어제·오늘 지출 인사이트와 합집합(`recovered_from_spend`), 221개.
- **카페24 앱 권한 추가(2026-09-18)**: OAuth SCOPE에 `mall.read_community`(게시판 읽기) 추가 — 에이전트 앱의 **광고 소재 담당**이 상품 후기 게시판(board 4, 알파리뷰가 카페24 게시판에도 동기화 — 실측 1만 5천여 건)을 읽는다. cafe24-analytics **`reviews`** 액션(`?product_no=`, 100건, 비밀글·답글·삭제 제외, 권한 없으면 `{error:"scope_missing"}` 200). 개발자센터 권한은 사용자가 켬(2026-09-18) — **'카페24 연동' 버튼으로 재연동해야 토큰에 반영**된다. meta-ads **`adperf`** 액션도 같은 날 신설(최근 30일 지출 광고 전부 + 클릭·노출·CTR·CPC·구매당 비용·ROAS, 꺼진 것 포함, 광고명 NFC 정규화, 30분 캐시 — 소재 유형별 효율 근거용).
...
- **휴가 관리 직원별 보기(2026-09-15 사용자 요청)**: 필터 바 '직원' select(`wm-lv-emp`, `wmLvEmpChange`) — 직원을 고르면 '올해 전체 보기'를 자동으로 켜서 연 단위로 다시 불러오고(`leave_list {year}`), 표는 그 직원만 + 상단 `wmLvPersonCard`(연차 총/사용/잔여 = 승인 건 기준 연차 1·반차 0.5, 여름휴가·병가 미차감 — employee_list 집계와 동일 규칙 / 종류별 건수 / 1~12월 칸에 날짜 칩). 상태 필터는 표에만 적용. 서버 변경 없음.
...
cr=admin, perf/adv/meeting_topics/meeting_notes/note_comments/note_likes=admin+staff, disp_season_out/profit_archive/ad_test_state=admin. **agent_reports=admin**(2026-09-10), **agent_actions=admin**(2026-09-11), **detail_reviews=admin**(2026-09-13, 상세페이지 점검 — 둘 다 소유는 ~/dnrb-agents).
...
### 취소 & 반품 (#cr, 관리자) — 소메뉴 2개: ① 취소 & 반품 비율 분석 ② 결제 주차별 취소·반품률
**② 결제 주차별 취소·반품률(`sec-cr-cohort`, 2026-09-11 사용자 요청)**: 별도 section-card, `showMenu('cr')`에서 sec-cr와 함께 표시(showMenu에 예외 한 줄), 헤더 클릭 `toggleSection('cr-cohort')`로 접기/펼치기. 진입 시 `crCohortInit()`이 기준일(어제 KST) 채우고 1회 자동 조회(`cohortweeks&weeks&days=14`, 서버 10분 캐시·새로고침은 nocache). KPI 4타일(최근 성숙 주 취소율/반품률, 성숙 주 평균, 이번 주 진행 중) + 주차 표(상태 칩 성숙/집계 중/진행 중, 경과일, 결제·취소·반품 건수와 비율, 성숙 주는 평균+0.5%p 초과 시 붉게) + 접이식 결제일별 14일 표. **원칙(사용자 지정)**: 그 주 결제 주문 중 언제든 취소·반품된 비율 = 결제 주 귀속, 성숙(14일↑)끼리만 비교. 에이전트 앱 반품 감시 보고서와 같은 데이터·기준.
...
- **ON 광고 열(2026-08-10, 관리자·MD — CS 숨김)**: 판매 성과 표에 상품별 **현재 활성 Meta 광고 수**. 숫자 클릭 → 광고명·**시작~어제 누적** 지출·구매당 비용·ROAS 팝업(`pa-modal`), 광고명 클릭 → 기존 소재 미리보기.
  - 데이터: `meta-ads?action=activeads` **1회**(Meta 2호출: 활성 광고 목록 + 어제까지 누적 인사이트, ~131개) — 상품별 개별 조회 금지(300호출 재앙). 팝업은 API 0회. limit 500, 넘으면 truncated 플래그. **activeads는 서버 10분 캐시(api_cache)** — 반복 조회 시 Meta 미호출. 실측: 검증 중 반복 호출로 Meta "User request limit reached"(400) 발생 → 캐시로 예방, 실패 시 열에 0 대신 '—' 표시(실패≠광고 없음). **⚠ 2026-09-15 실사례**: 어제까지 되던 인사이트 호출(since 2024-01-01 + `ad.effective_status` IN ACTIVE 필터)을 Meta가 갑자기 400 'Invalid parameter'로 거부 — 광고 목록 호출·짧은 기간 필터 호출·긴 기간 무필터 30개는 정상, 긴 기간 무필터 전량은 500 "reduce the amount of data". → 필터 호출 실패 시 **활성 광고 id를 50개씩 `ad.id IN` 필터로 나눠 받는 폴백**(223개 → 5회, 14초), 응답 `source: filtered|by_ids`. 에이전트 상품 전략 보고서가 이날 "광고 0개"로 오판한 원인이었음(에이전트 쪽도 실패=모름으로 고침). 같은 날 함께 고침: ① `/ads`·`/adsets` 목록은 graphGetAll로 페이지 끝까지(activeads·hierarchy) ② **testads·syncexport의 테스트 세트 200개 상한 제거**(실측 테스트 세트 372개 → 172개 세트의 소재가 테스트 소재 화면·상품관리 연동에서 빠지고 있었음, 세트 id 100개씩 나눠 조회). ⚠ **광고명은 NFD(자모 분해)로 저장된 것이 많다**(워크스페이스 광고관리자가 API로 만든 소재) — 파이썬/JS에서 `'우디' in name` 같은 NFC 포함 검사와 Meta의 name CONTAIN 필터 모두 안 걸린다. 이름으로 찾을 때는 NFC 정규화(조사 중 이걸 놓쳐 "목록에서 빠진 광고"로 오인한 전력, 2026-09-15).
  - **매칭 규칙**(`paKey`/`paVerTok`/`paGroups`/`paPickBest` — 2026-08-26부터 광고관리자 실결제 수와 공용): 핵심명 = **앞·뒤 괄호·대괄호를 전부(반복) 벗긴** 상품명(2026-08-26 수정 — 예전엔 앞 괄호 1개+뒤 괄호만이라 "[Best] (2사이즈) 클레르 블라우스"의 핵심명이 안 나와, 클레르 광고들이 매칭 실패하거나 겨울 '(기모ver.) 클레르'에 오매칭됐음), 공백·'ver.'점 무시 비교, 핵심명 3자 미만 제외. 광고 1개는 **가장 긴 핵심명 1개에만** 배정(부분 겹침 오매칭 방지). **같은 핵심명 상품이 여럿이면**: 광고명에 ver 토큰(여름ver·기모ver 등)이 있으면 그 버전에, **없으면 'ver 없는 기본판'에 배정(그룹에 기본판이 딱 하나일 때, 또는 2026-09-12 우세 규칙: 기본판이 여럿이면 기간 판매량 70%↑인 상품 — 삭제된 옛 상품 1393이 애널리틱스 이력에 남아 세러데이 나그랑 광고 11개가 미배정되던 사례** — 2026-08-26 개선, 예전엔 무조건 미배정이라 기본판 광고가 영영 안 잡혔음), 기본판이 여럿이면 미배정. **⇒ 운영 규칙: ver 상품 광고는 광고명에 ver 표기 필수** (안 쓰면 기본판으로 붙음). 검증(2026-08-26, 활성 114개 전수 신구 비교): 매칭 102→108, 기존 매칭 소실 0, 변경 7건 전부 클레르 오매칭 교정, [Best] 클레르(1383) ON광고 0→5. **실측: 광고명은 상품명 괄호를 그대로 복사하지 않음**("(2사이즈,여름ver.)" → 광고엔 "여름ver"만 또는 생략)이라 괄호 통째 매칭은 전멸함. 'silver'의 ver은 버전 아님((?<![a-z])ver). **비교는 한글을 자모로 풀어서**(paJamo) — 맥에서 광고명이 "ㅍㅡㄹㅔㅅㅣ ㅎㅜㄹ ㅌㅣㅅㅕㅊㅡ"처럼 조합 안 된 자모로 등록되는 실사례가 있어(2026-08-10), "프레시"→"ㅍㅡㄹㅔㅅㅣ"로 풀면 깨진 이름과도 일치한다. 검증: 131개 광고 중 64개가 25개 상품에 배정, 오매칭 0(포켓/쿨프리 분리), 썸머 클레르 13·뉴 내티 3·여름ver 레이스 클레르 1.
...
- **제작처 태그 `made_products`**(product_no PK, origin cn/kr, lead_days 덮어쓰기, moq 최소 주문 수량(2026-09-07) — db 프록시 읽기·쓰기 admin+staff. 처음엔 admin만 쓰기였다가 2026-09-03 사용자 요청으로 MD에도 개방, 커스텀 차단 규칙 제거). 리드타임 기본 **중국 10일 / 국내 9일(영업일 7일 환산)**.
...
- **판정 규칙 v3 (2026-09-07 사용자 지정, 커버 목표일 폐지)**: 리드타임 기본 **중국 10일 / 국내 5일**(이전 9일). 옵션별로 **안정재고 = 일평균 ≥3 → 일평균×리드타임, <3 → 일평균×3일**(올림). **주문 필요 = 가용재고 < 안정재고, 권장 주문량 = 안정재고 − 가용재고**(옵션 합), 임박 = 안정재고는 넘지만 3일치 이내 여유. **최소 주문 수량(MOQ, `made_products.moq` 컬럼 추가)**: 권장이 MOQ보다 작으면 MOQ로 올리고 '최소 주문 수량 적용' 표시. 표에 안정재고·최소 주문 열 추가(옵션 행에도 안정재고 ×N일). 상품관리 `MadeCheckClient`도 동일 규칙 — 함께 수정할 것.
- **옵션 매칭·판정(v2, 매칭 부분은 유지)**: 카페24 option_value ↔ CSV 옵션을 stkOptTokens→stkSameOpt(정확)→stkLooseOpt(느슨, '표기차이' 표시)→옵션 없는 상품은 단일 행 폴백으로 매칭. 옵션별 judgeOne: 소진일수 = 가용÷7일평균 → ≤리드 🔴주문 필요 / ≤리드+3일 🟠임박 / 그 외 🟢, **옵션별 권장 = ceil(7일평균×(리드+커버목표)−가용)**, 커버 기본 14일(localStorage `dnrb_made_cover`). **상품 행 판정 = 옵션 중 최악(과 상품합계 판정 비교), 권장 = 옵션 합('옵션 합계' 표기)**. 상품 행의 `옵션 N개 ▾` 버튼(madeToggleOpts, madeState.open Set) → 접이식 옵션 상세 행(옵션/7일평균/30일/가용/판정/권장). 정렬 = 심각순.
...
- **소메뉴 2개 + 공용 파일(2026-09-21 사용자 요청)**: #madechk가 탭 **① 재고·입고 점검(맨 위, 기본) ② 주문 점검**으로 나뉨(`MADE_TABS`/`madeSetTab`, 주문 점검 탭에 들어갈 때만 카페24 수집). 파일은 상단 공용 드롭존 `handleMadeFiles(files)` 하나 — **파일명으로 자동 구분**(`stk_stockList*`/.csv → cell, `제작상품관리*`/.xls(x) → ez, 여러 개 동시 가능) → `madeParseCell`(EUC-KR, 헤더명 인식: 상품명·옵션명·현재재고·안정재고·미발송주문수·**바코드번호(서식) `="10304120012"`→코드**·사입상품명·품절여부) / `madeParseEz`(+ **쇼핑몰상품코드 S열**) → **서버 `made_check_files`(kind PK cell|ez, rows JSONB, migrations/0015)에 저장 → 팀 전체 공유, 다음에 열 때 자동 로드(`madeFilesLoad`)**. 셀메이트 API는 유료(100만원)라 CSV 방식 유지 — 대신 "한 번 올리면 모두가 봄"으로 답함. `madeApplyFiles`가 저장 행으로 주문 점검의 `madeState.stock/ez`(예전 구조 그대로)도 만들어 두 탭이 같은 파일을 씀. 예전 `handleMadeCSV/handleMadeEz`는 래퍼.
- **재고·입고 점검 탭(`madeStkCompute/madeStkRender`)**: 대상 = 상품명에 made/자체제작(`madeIsMade`) + **지정 상품**(`made_watch_products` name_key=stkNorm(상품명), 팀 공유, 패널 `madeWatchPanelRender/madeWatchToggle` — 올린 CSV의 비대상 상품을 검색해 +). **옵션 매칭은 셀메이트 J열 바코드 = 이지픽 S열 쇼핑몰상품코드**(이름 매칭 아님). 옵션별 가용=재고−미발송, 이지픽 미입고=코드 같은 이지픽 행 잔여 합(취소 제외), 입고 후=가용+미입고, **부족 = 현재 재고 기준 = max(0, 미발송−재고−미입고)**(2026-09-21 사용자 정정: 안정재고 기준 아님) + **주의 배지 = 입고 후 5개 미만 또는 셀메이트 안정재고(E열) 미달**(옵션 행 노란 배지·상품 행 '주의 N옵션', 칩 '주의 있음', 타일 '주의 옵션', 엑셀 '주의' 열). 요약 타일·칩(전체/부족 있음/지정)·검색·엑셀(`madeStkExport`, xlsx-populate). 이지픽 예외 표: **상품코드 없음**(S열 공란)과 **상품코드 매칭 안 됨**(코드는 있으나 올린 CSV에 없음 — CSV가 일부 상품만 내려받은 경우). 검증(2026-09-21 실파일): 셀메이트 75행 전부 코드 있음·이지픽 22행 중 19 코드 → 점검 23상품 75옵션, 코드없음 3(BS-0180), 매칭안됨 8(CSV가 '자체제작'만 담겨 'made' 상품 누락 — 사용자에게 안내), 클래식 카라 블랙 숏 재고0·미발송22·미입고40→입고후18·안정5→부족0.
- **이지픽 제작상품 엑셀 = 입고 예정(2026-09-09 사용자 요청)**: 두 번째 업로드 칸 `#made-ez`(`handleMadeEz`) — 이지픽 '제작상품관리(전체보기)' .xls를 **cdnjs SheetJS 0.18.5 지연 로드**(`loadSheetJS`, xlsx-populate는 BIFF .xls를 못 읽음)로 파싱. 열은 헤더명으로 인식(상품명 G·옵션 H·주문수량 J·입고수량 K·잔여수량 M·처리상태 O·쇼핑몰상품명 R), **상품 키 = 쇼핑몰상품명(=카페24 상품명) 없으면 상품명**, 처리상태 '취소' 행 제외, **입고 예정 = 잔여수량**(비어 있으면 주문−입고, 음수 0). 매칭 `madeEzOf`: 정규화 일치 → 핵심명(`madeCoreName`, 괄호 태그 제거) 포함 관계 중 가장 긴 것; 옵션은 셀메이트와 같은 토큰 규칙. **판정·권장 주문량은 (가용재고 + 입고 예정) 기준**(이미 주문한 수량을 또 주문하지 않도록), 표에 '입고 예정'·'입고 후 과부족 = (가용+입고 예정) − 안정재고'(부족 N장 빨강 / 여유 N장 초록) 열 추가, 옵션 행에 '주문 N · 입고 M' 보조 표기, 요약 타일 '입고 예정' 합계. 셀메이트 CSV에 '안정재고' 열이 있으면 계산값과 다를 때 '셀메이트 설정 N'으로 참고 표시(계산은 v3 규칙 유지), '발송필요' 헤더도 미발송 별칭. 검증: 샘플 xls 20행·6상품·잔여 751 — 세러데이 그레이 101(100+1)/네이비 200/버건디 100, 후드집업(입고 완료) 0, 핵심명 폴백 매칭 OK.
- **셀메이트 수동 매칭(2026-09-09 사용자 요청)**: CSV를 올렸는데 상품명으로 못 찾은 상품은 판정 **'매칭불가(미등록/이름다름)'**(state `nomatch`, 보라 배지, 요약 타일)로 분류. 상품명 셀의 **'셀메이트 상품 찾기'**(`madeMatchOpen` 팝업, CSV 상품명 검색 → '이 상품으로 매칭')로 연결하면 `made_products.stk_name`(셀메이트 원본 상품명, 2026-09-09 컬럼 추가 — 관리 API `alter table … add column if not exists`)에 저장돼 **상품관리 시스템과 공유**. 조회는 `madeStockOf`: stk_name 우선, CSV에 없으면 상품명 자동 매칭 폴백(+'이 CSV에 없어요' 경고). 자동 매칭된 상품도 '수동 매칭/매칭 변경'으로 바꿀 수 있고 팝업에서 '매칭 해제'. 셀메이트 미등록 상품은 검색 결과가 없으니 그대로 매칭불가로 남는다(등록 후 CSV 재업로드). stock 엔트리에 `name`(원본명) 보관.
- **⚠ 괄호 표기 차이 폴백은 넣지 말 것(2026-09-09 사용자 결정)**: 셀메이트 옵션 `S(55~66)` vs 카페24 `S(55~77)`처럼 괄호 안만 다를 때 같은 옵션으로 보는 `stkCoreOpt`를 안정재고 편성·주문 점검·npm에 넣었다가 사용자 요청으로 **전부 되돌림**("다른 문제가 나타날 수 있다"). 해법은 **셀메이트 옵션명을 카페24와 동일하게 수정**하는 것(사용자가 직접 정리함). 매칭은 정확→느슨(stkLooseOpt) 2단계 유지.
...
- **미리보기 형식 전환(2026-09-01 사용자 신고 해결)**: 릴스 게시물 기반 광고는 기본 피드(INSTAGRAM_STANDARD) 미리보기가 **"지원되는 화면 비율은 9:16..." 안내문만** 나온다(실사고 — 기존릴스_test 소재). 서버 preview `fmt` 파라미터를 4종(feed/reels/story/desktop)으로 확장, 클라 **공용 로더 `metaPrevLoad`**(형식별 메모리 캐시·늦은 응답 버림)가 모달에 **[피드][릴스][스토리] 칩** 표시 + **이름에 '릴스'가 들면 자동으로 릴스 형식** 시작. 성과 모달(showMetaPreview)·베스트소재 경량 모달(admgrBestOpen) 둘 다 이 로더 사용(중복 fill 함수 제거). 검증: 릴스 소재가 실제 영상 프레임으로 렌더, 칩 전환·자동 선택 정상. **릴스 잘림 원인·해결(2026-09-04 사용자 신고)**: Meta가 INSTAGRAM_REELS 미리보기 iframe을 **274×213 가로형 상자**로 내려준다(피드 320×525·스토리 320×567은 정상) — 내용은 세로 영상이라 상자 안에서 잘리고 스크롤바가 생겼다. 내용이 iframe 크기에 맞춰 다시 그려지므로 metaPrevLoad가 릴스이거나 가로형(h<w) 상자면 **320×640으로 강제** → 실측 전체 표시. `max-width:100%`로 상자만 줄이는 건 내용을 못 줄이니 해결책이 아니다.
...
- **⚠ 메뉴 숨김(2026-09-11 사용자 결정)**: 광고관리자 기능은 친구가 만든 별도 앱 **ad-dashboard**(공개 저장소 github.com/qkralsrb03138668/ad-dashboard, GitHub Pages `https://qkralsrb03138668.github.io/ad-dashboard/`, 친구 Supabase `pydxcqfztjogmztvayux`, 친구 서버가 Meta 데이터 5분마다 수집 — 우리 토큰·시크릿 공유 없음)로 대체. 메뉴 `menu-addash`(`adDashOpen()`, **관리자 + 허용 목록 `ad_dashboard_users`(migrations/0009, 직원 관리 화면 칩 `adDashPanel/adDashToggle`, admin만 쓰기·읽기 전원, 서버 `audAllowed`가 sso_issue와 verify 양쪽에서 강제 — 빼면 즉시 차단)** — 처음엔 로그인 전원이었다가 2026-09-11 사용자 지정으로 허용제). **친구 쪽 구현 완료·왕복 검증(2026-09-11, 친구 커밋 8cc1238)**: QA 직원을 허용 목록에 넣고 코드 발급 → 친구 페이지 `#sso=` 진입 → 로그인 화면 없이 'QA직원 (마케터)' 칩, 주소에서 코드 제거, 친구 서버 auth-admin me 200 / 허용 해제 후 새 토큰으로 친구 서버 호출 → '로그인이 필요합니다'(우리 verify 403) + 친구 클라가 저장 세션 삭제. **세부 권한(2026-09-11 사용자 요청, 친구 확인)**: `ad_dashboard_users.perms` JSONB(migrations/0010) = `{menus:[home compare ptest atest abest admgr upload perf data shoot], actions:[toggle budget upload creative delete]}`(친구 앱 메뉴 키 그대로 — 처음 list·test로 냈다가 친구 확인으로 atest 테스트 소재·abest 베스트소재로 교정, migrations/0011이 기본값·기존 행 치환. **최종 메뉴(2026-09-11 확정, 사진 기준)**: 소재(ptest '소재 업로드'·atest 테스트 소재·abest 베스트소재) / 분석(home 대시보드·compare 소재 비교, 접힘 그룹) / Meta 연동(admgr 광고관리자·upload 광고 업로드) / 카페24 연동(perf) / 관리(data) / 현장(shoot) — 키 10개는 동일, ptest 표시명만 '광고소재 대시보드'→'소재 업로드'. 권한 모달은 이 그룹·순서로 표시(`AD_PERM_MENUS` 3번째 원소 = 그룹). 친구 쪽 구현 완료 커밋 3a34d3a·9d205ec: 미디어 업로드는 upload 외 creative 권한으로도 통과, cafe24-perf는 admin 또는 perf 메뉴 허용자). 직원 관리 칩 옆 '권한' 버튼 → 체크박스 모달(`adDashPermOpen/Save`, 프리셋 보기 전용/전체), 새로 허용 = 보기 전용 기본. 서버 `audAllowed`가 perms를 돌려주고 **verify·sso_redeem 응답에 `perms` 포함**(admin은 전부, 알 수 없는 키는 걸러냄 `normPerms`). 친구 앱이 화면 숨김 + 서버 함수(meta-budget·meta-upload 등)에서 검사 — 가이드 2탄 전달. ⚠ 친구 서버가 우리 auth를 부를 때 **apikey(anon) 헤더 필수**(게이트웨이가 없으면 401) — anon 키는 공개 레포 config.js 값이라 내장 OK, 권한은 토큰이 결정. → `auth sso_issue {aud:'ad-dashboard'}` 60초 코드 → 저쪽이 `sso_redeem`으로 전용 토큰 → 매 요청 `auth verify`. 내장 #admgr은 **'광고관리자(기존)' 메뉴로 `ADMGR_LEGACY_USERS=['admin']`(**사용자 본인 계정 = 아이디 `admin`** — 처음 박민규 계정으로 잘못 잡았다가 사용자 정정)에게만 노출**(2026-09-13 사용자 요청 "나만"; `admgrLegacyAllowed()`, showMenu 진입도 같은 조건으로 막음 — 코드·서버 액션·ad_test_state·예산 예약 전부 유지). 계정 추가 = 배열에 아이디 추가. 이 앱은 친구 저장소라 우리 CLAUDE 규칙 밖 — 공동 개발하려면 친구 초대 필요(공개 저장소라 읽기는 가능).
...
- **담당자/주문자 분리(2026-09-10 사용자 요청 — 알바 주문도 등록)**: `requester_id/requester_name` = **담당자(입력자)** — db 프록시가 POST 때 `requester_name = me.name`으로 **서버에서 덮어써** 클라이언트가 못 바꿈(화면은 readonly 회색 칸). 새 컬럼 `orderer_name`(migrations/0008) = **주문자** — 폼 기본값 본인 이름(`purchFillNames`, 비어 있을 때만 채움), 직접 타이핑해 변경, 비우면 서버가 담당자 이름으로. 표: 담당자(m-hide)·주문자(주문자≠담당자면 아래에 '담당 OOO' 보조 표기). 알림·삭제·본인 행 판정은 계속 requester_id 기준. 검증: QA staff가 requester_name을 위조해 POST → 저장값은 로그인 이름 / 주문자 '알바 홍길동' 저장 / 주문자 공란 → 담당자 이름.
...
- **소메뉴 ② 불량·오배송 처리(2026-09-22f 사용자 요청)**: `#rscan-tabs`(uiTabs, `RSCAN_TABS` scan/issues, `rscanSetTab`, pane `.rscan-pane`), 서버 `rscan_issues`(권한 admin/staff/cs) = **최근 30일(어제까지, 주문일 기준)** 인덱스(신선·≥30일이면 재사용, 아니면 30일로 재생성) 항목 중 `rscanJudge` 경고(alert/warn) 건. 네이버페이 주문은 `rscan_naver`(상품주문번호→사유)로 사유 보강. 응답 `{issues[{key,kind,order_id,buyer,receiver,items,reason,reason_type,naver_reason,alert,...}], done{key:{by,at}}}` — 실측 30일 1,424건 중 62건·16초(목록 생성 포함). **처리 상태는 `rscan_done`(migrations/0017, key=`${kind}:${order_id}:${claim_code}`, db 프록시 전원 읽기·쓰기)** — 체크박스 `rscanIssueToggle` 낙관적 갱신 후 upsert, 실패 시 원복. 필터 칩 처리전/처리완료/전체(건수), 탭 배지=처리전 건수, 열 = 처리·구분(반품/교환 + 불량/오배송 배지, warn은 '의심')·주문번호(+카페24/네이버 바로가기)·이름(구매자, 수령인 다르면 병기)·상품(옵션·수량)·사유(m-hide, **2026-09-23: `.rscan-reason` 한 줄 …요약 + title 툴팁 + 클릭 펼침** — 긴 사유가 표를 늘어뜨리던 것). **접수일은 처리·구분 사이(작은 글씨, YY-MM-DD, 2026-09-23 사용자 요청)**, 표 `.rscan-issues-tbl` 글씨 .78rem·여백 6/7px로 축소. 서버가 done 조회 시 done_at 최근 120일만 읽음. **품목 = 접수 품목만(2026-09-22g 사용자 지적)**: rscanBuild의 `its`가 claim_code 없는 정상 품목까지 포함해 주문 상품이 다 보였음 → ① 해당 claim_code 품목 → ② 없으면 order_status R*/E* 품목 → ③ 전체(안전망). 인덱스 재생성 후 반영(24건에서 품목 수 감소 확인, 0품목 없음). **엑셀 다운로드** `rscanIssuesXlsx`(현재 구분·상태 필터에 보이는 주문, 접수 품목당 1행·주문 정보는 첫 행에만, 22열, xlsx-populate).
  ...
  - **목록 자동 갱신(2026-09-22k 고도화 2번)**: pg_cron 잡 `rscan-index-15min`(jobid 7, `*/15 23,0-11 * * *` UTC = **08:00~20:45 KST 15분마다**) → `net.http_post(cafe24-analytics?action=rscan_build&days=90, Authorization Bearer anon, x-cron-secret, timeout 120s)`. cafe24-analytics는 `x-cron-secret`(CRON_SECRET)을 **rscan_build 액션에만** admin으로 인정(`viaCron`). 인덱스가 늘 20분 이내라 조회·처리 목록이 재생성 없이 바로 뜸(90일 45초짜리 빌드는 서버가 미리). 잡 수정은 관리 API SQL(`cron.unschedule` 후 `cron.schedule`, 비밀값은 jobid 1 command에서 재사용).
  - **아침 엑셀 미등록 경고(2026-09-22k 3번)**: `rscanStatusRender`가 `rscan_naver` 마지막 업로드의 한국 날짜 ≠ 오늘이면 `#rscan-naver-warn`에 빨간 띠(마지막 등록일 + '엑셀 올리러 가기' 스크롤 버튼), 오늘이면 초록 한 줄.
  - **소메뉴 ③ 불량 다발 상품(2026-09-22k 6번)**: 서버 `rscan_issues` 응답 `products[]`(정리된 건 포함, 접수 단위 — 접수 안 같은 상품 여러 줄은 1건·수량 합산: product_no·name·claims·defect·misdeliver·suspect·returns·exchanges·qty·last_date·reasons(최대 3)·orders) — rscanBuild 품목에 `product_no` 추가(재생성 후 반영, 실측 76개 상품 전부 번호 있음). 화면 `rscanProductsRender`(순위·상품·접수 배지·반품/교환·수량·최근·사유 예시), **관리 상품 지정**(admin·MD만, `rscanWatchAdd` → `return_watch` upsert reason '불량·오배송 접수 N건 (반품 스캔 최근 60일)', 지정 후 `rwState.watch=null`로 반품 관리 재로드 유도, 이미 지정은 '관리 중/판매 중단' 표시 — `rscanWatchLoad`).
...
- **이름·주소 찾기(2026-09-23 사용자 요청)**: 고객이 우체국·편의점으로 직접 보낸 반품은 카페24에 `return_invoice_no`가 없어(실사례 주문 20260915-0006998 — `use_pickup=F`, 최근 한 달 수거 전 212건 중 1건, 완료 647건 중 32건) 송장 스캔이 안 맞는다 → 스캔 입력 아래 `#rscan-find` 칸: 두 글자부터 450ms 디바운스 자동 조회(`rscanFindAuto/rscanFind`), 서버 `rscan_find&q=&days=`(권한 admin/staff/cs, 최소 30일) = 인덱스에서 **수거 전 반품·교환만**(반품: 반품완료·환불전 제외 / 교환: 교환완료 제외 / 철회 제외) 구매자·수령인·배송지 주소(공백 제거·소문자)·전화(숫자 4자리 이상) 부분 일치, 최대 30건, 전화는 가운데 마스킹. 인덱스 항목에 `address`(receivers[0].address1+2)·`phone` 추가(재생성 후 반영). 결과 카드에 카페24/네이버페이센터 버튼(`rscanLinkBtns`) + N 상품주문번호. **결과가 1건일 때 Enter로 바로 열림**(`rscanOpenLink` — window.open은 사용자 동작 안에서만 가능해 자동 조회 결과는 버튼). 없음 안내 문구에 힌트 추가. 검증: 수령인 전체·주소 어절·주소 뒷 6자·전화 뒷 4자리 모두 같은 수거 전 건 검색됨(2026-09-23). 스캔한 송장을 주문에 연결해 기억하는 부분(원안 1번의 나머지)은 **사용자가 제외**.
...
- **바로가기 버튼(2026-09-22d)**: 결과 카드에 '카페24 주문'(항상)·'네이버페이센터'(N Pay 주문만, 클릭 시 상품주문번호 클립보드 복사 토스트) — `rscanLinkBtns/rscanOpen`(data- 속성, 새 탭). 주소 형식은 `rscan_settings.settings.cafe24_url / naver_url`(관리자 '경고 기준' 패널 하단, `{order_id}`·`{product_order_no}`·`{kind}` 치환, https 필수) — 기본값 `RSCAN_URL_DEFAULTS`: 카페24 `…/admin/php/shop1/s_new/order_detail.php?order_id={order_id}&menu_no=78`(구 관리자 주문 상세 팝업 — 공개 코드(yungsuKo/oneroom-webhook-server)에서 같은 형식 확인, 2026-09-23 `menu_no=78` 추가. 직원 PC에서 '연결할 수 없는 페이지'가 뜬 보고 있음 → 우리 앱은 역할 제한 없음, 카페24 관리자 로그인·부운영자 주문 권한 문제일 수 있음. **사용자 실검증 대기**), 네이버 `https://admin.pay.naver.com/o/v3/claim/{kind}`(반품/교환 관리 목록 — 비로그인 시 로그인으로 302라 존재 확인됨). **'반품수거완료 + 수취인명' 상태 딥링크는 불가(2026-09-23 사용자 확인: 타일·조건을 골라도 주소가 안 바뀌는 SPA)** → 버튼이 목록을 열며 **카페24 품목의 네이버페이 상품주문번호(`naver_ids[0]`)를 클립보드에 복사**(2026-09-23 사용자 결정 — 수취인명 아님)하고 '상세조건을 상품주문번호로' 토스트. 스캔 카드·처리 목록에 `rscanNaverIdsHtml`로 번호 전부 + 개별 복사 버튼 표시. 더 원하면 Tampermonkey 스크립트(엣지 지원)로 페이지 열릴 때 타일 클릭·조건 선택·이름 입력을 자동화할 수 있으나 DOM 확인 필요 — 사용자 미요청. 카페24 주소가 틀리면 사용자가 실제 주문 상세 주소를 알려 주고 설정 패널에서 바꾸거나 기본값을 수정.
...
- **출퇴근 기록 엑셀(2026-09-09, 기존 프로그램과 동일)**: `wmAttExportRows()`+`wmExportAtt()` — work-manager `/api/attendance/export` 양식 그대로(1행 제목 'YYYY년 M월 출퇴근 기록', 3행 헤더 10열 이름·구분·날짜·요일·출근·퇴근·총 근무시간·식대공제·실 근무시간·비고). 직원 필터 선택 시 그 달 **모든 날짜**(기록 없는 날 빈 칸), 전체면 기록 있는 날만 날짜·이름순. 식대공제 = `wmMeal`(급여와 동일 규칙). xlsx-populate로 생성, 시트명 'M월', 파일명 `YYYY년_M월_출퇴근기록[_이름].xlsx`. 검증: 2026-08 전체 182행·개인 31행.
- **급여 명세서 선택 인쇄(2026-09-09)**: 급여 표 첫 열 체크박스(`.wm-sal-chk`, 머리 전체선택 `wm-sal-all` indeterminate 지원) → '선택 명세서 인쇄 / PDF (N명)' 버튼(`wmSlipPrintSelected`). 행별 'PDF' 버튼은 그 사람만.
- **급여 명세서 인쇄/PDF(2026-09-09, 사용자 요청 "기존 시스템처럼")**: 명세서 HTML을 `wmSlipHtml(r, forPdf)`로 분리(모달·인쇄 공용, 금액 계산 무변경 — 1원 불변). **양식은 기존 work-manager `Salary.jsx SlipView`를 그대로 옮김**(급여 명세서/YYYY년 M월분 헤더, 성명·구분 배지·지급 기간·총 근무시간·시급, 항목/금액 표, 주휴수당 '(해당 주: N주)', 식대 '점심 제공일', 총 지급액·실지급액, 주휴수당 상세+안내 4줄, 하단 '※ 본인 외 공유 금지'). 스타일은 `WM_SLIP_CSS`(.wm-slip 스코프)로 모달에는 `<style>` 주입, 인쇄 문서에는 삽입. **방식도 기존과 동일 = 브라우저 인쇄**: `wmSlipPrint(indices)`가 명세서만 담은 새 창(`wmSlipPrintDoc`, @page A4 14mm, 사람마다 page-break)을 열고 폰트 로드 후 자동 `window.print()` → 사용자가 'PDF로 저장' 선택(텍스트 PDF, 파일명 기본값 = 창 제목 `급여명세서_YYYY-MM_이름|전체`). 버튼: 급여 탭 '명세서 전체 인쇄 / PDF', 표 각 행 'PDF', 모달 '인쇄 / PDF 저장'. PDF/인쇄본에는 '관리자 전용' 계좌 블록 제외(forPdf). ⚠ 함정: 인라인 스크립트 안 문자열에 `</script>`를 그대로 쓰면 HTML 파서가 스크립트를 거기서 끊어 이후 함수가 전부 undefined가 된다(실사고) — `</scr${'ipt'}>`로 분리. 문법 검사는 `<script>…</script>` 블록 단위로. (html2canvas+jsPDF 이미지 PDF 방식은 사용자 요청으로 폐기.)
...
- ✅ **서류 발급: PDF 직접 저장 + 출력 요청(2026-09-21 사용자 요청)**: 마이페이지 → 서류 발급의 재직증명서·차량 등록 요청서 버튼이 '발급(인쇄·PDF 저장)' 하나에서 **PDF 저장 / 인쇄 / 출력 요청** 셋으로.
  - **PDF 저장** `docSavePdf(title, prefix, inner, opt, person, fileName)`: 인쇄용 HTML(`docHtml` — `docOpen`에서 분리)을 화면 밖 794px iframe에 그려 **html2canvas(2배) → jsPDF A4**(cdnjs에서 필요할 때만 로드 `loadPdfLibs`). 래스터 PDF(약 160KB, 1쪽 초과 시 쪽 나눔). 파일명 `재직증명서_이름_날짜.pdf`. 검증: 1588×2246 캔버스·297mm 1쪽·내용 픽셀 확인. 인쇄 창 방식(`docOpen`)은 그대로.
  - **출력 요청**: `wm-me doc_request {doc_type cert|car, use, car_no, car_model}` → `wm_doc_requests`(migrations/0014, 같은 서류 '요청됨' 중복 409) + 관리자 전원 앱 알림·푸시(notify 경유, 제목 '재직증명서 출력 요청'). 직원은 '내 출력 요청' 표에서 상태(요청됨/출력 완료/취소)·`doc_request_cancel`.
  - **관리자**: 근무 관리 → **'서류 요청' 탭**(`wmDocReqLoad/Render`, 기본 필터 요청됨) — 인쇄(`docOpen`)·PDF(`docSavePdf`)를 **관리자 PC에서** 직원 대신 실행. 사람 정보 `wmDocPerson` = wm_employees(name·birthday·hire_date) + **소속은 app_users 역할**(`list_users` 캐시 `wmState.appUsers`, 미연결이면 직원/아르바이트) — 마이페이지 발급본과 동일 내용(문서 공용 `docPersonRows/certInner/carInner/CAR_STMT`). '출력 완료'(메모 선택) → `wm-admin doc_request_done` → 직원에게 "받아가세요" 앱 알림+푸시 / 취소 / 되돌리기. 검증(QA 계정·직원 1022, 전부 삭제): 요청 200·중복 409·차량번호 없음 400·직원 취소·관리자 목록·관리자 문서(소속 CS팀·생년월일·재직기간)·출력 완료 알림.
- ✅ **출장 여비 신청서 제출·확인 + 개인 사비 지출·영수증(2026-09-17 사용자 요청)**: 마이페이지 → 서류 발급 → 출장 여비 신청서가 '발급(인쇄·PDF)' → **'입력완료'(`myTripSubmit`)** 로 바뀜 — 서버 저장 후 관리자 확인을 받는 흐름.
  - **개인 사비 지출 내역**(`myTripExpRows`/`myTripExpRender`): 날짜·지출 내용·금액·메모 행 추가 + 행마다 **영수증 첨부**(PDF·JPG·PNG·WEBP·HEIC, 8MB) → `wm-me trip_receipt_upload`로 즉시 업로드하고 받은 id를 행에 붙인다(제출 시 그 id만 연결, 남의 영수증 id는 서버가 걸러냄).
  - **DB/스토리지**(migrations/0012): `wm_trip_claims`(ot·expenses JSONB, trip_pay·expense_total·ot_total_min, status submitted|confirmed|returned, reviewed_by/at·review_note) + `wm_trip_receipts` + **비공개 버킷 `wm-receipts`**(경로 `{employee_id}/{uuid}.{ext}`). 공개·서명 URL 없이 함수가 바이트 중계(`Cache-Control: private, no-store`).
  - **서버 재계산**: `wm-me trip_submit`이 단가표(TRIP_PER domestic 10,000 / intl_short 20,000 / intl_long 40,000)·초과근로(식사 -60분 후 15분 올림)·사비 합계를 **다시 계산**해 저장 — 화면 값을 믿지 않는다. 제출 시 관리자 전원에게 알림(link_menu 'wm').
  - **관리자**: 근무 관리 → **'출장 여비' 탭**(`wmTripLoad/wmTripRender`, 기본 필터 '확인 대기') — 직원·출장·출장비·초과근로·사비 지출(건별 내역 + 영수증 열람 버튼, 영수증 없는 건은 주황 표시)·합계·상태, **확인 완료 / 보완 요청(사유 필수, 직원 알림) / 되돌리기 / 삭제(영수증까지)**, 인쇄. 영수증은 `wm-admin trip_receipt` 중계로만.
  - **상세 보기(2026-09-18 사용자 요청)**: 관리자 표의 **행 클릭(또는 '상세' 버튼) → `wmTripDetail` 모달** — 출장 내역, **초과 근로 행별 날짜(요일)·시작~종료(자정 넘김 '다음날')·경과 시간·식사 공제·실제 근로·15분 올림(+N분)·업무 내용**과 합계, 사비 지출 표(영수증 열기), **이미지 영수증 썸네일**(함수 중계 blob → object URL, 닫을 때 revoke), 청구 합계, 관리자 확인 이력, 하단에 인쇄·되돌리기·보완 요청·확인 완료. 표 안 버튼들은 stopPropagation. ESC·바깥 클릭으로 닫힘.
  - **수정·재제출(2026-09-21 사용자 요청)**: 내 신청 내역의 '수정'(확인 대기) / '수정해서 다시 제출'(보완 요청) → `myTripEdit(id)`가 양식에 저장값(초과근로·사비·영수증 연결 포함)을 채우고 상단에 수정 배너(관리자 보완 사유 표시, 수정 취소) + 버튼 '수정 제출' → `wm-me trip_update`(본인 + submitted/returned만, confirmed는 409) → 서버 재계산 후 status submitted·reviewed_* 초기화·`review_note`→`prev_review_note` 보관·`resubmit_count`+1·`resubmitted_at`(migrations/0013), 영수증 연결 갱신(빠진 건 해제), 관리자 알림 '보완 후 재제출'. 관리자 표·상세에 '재제출 N회'와 '직전 보완 요청' 표시. 확인 완료 건은 관리자가 '되돌리기' 해야 직원이 수정 가능.
  - **알림 = 앱 알림 + 웹 푸시(2026-09-21 사용자 요청 "재제출하면 나한테도 알림")**: 제출·재제출(wm-me `notifyAdmins`)과 확인 완료/보완 요청(wm-admin trip_review)이 `notifications` 직접 삽입 대신 **`notify` 함수를 내부 호출**(헤더: 서비스키 apikey + 요청자의 `x-auth-token` 그대로 전달 → notify의 verifyAuthToken 통과) — 앱 종 알림 + push_subscriptions 전 기기 웹 푸시. notify에 `title` 파라미터 추가(없으면 기존 '○○님이 나를 언급했어요'). notify 실패 시 예전처럼 행만 삽입. 관리자 기기: admin 2대·박민규 1대·김다나 0대(2026-09-21 기준 — 김다나는 휴대폰에서 알림 허용을 켜야 푸시가 감).
  - **초과근로 엑셀(2026-09-18)**: `wmTripOtExport(onlyId?)`(xlsx-populate) — 탭 필터 바 '초과근로 엑셀'(보이는 신청서 = 상태·직원 필터 반영) / 상세 모달 '초과근로 엑셀'(그 신청서만). 1줄 = 초과근로 1건: 직원·출장지·기간·날짜·요일·시작·종료·다음날·경과·식사 공제·실제·인정(글자) + 실제(분)·인정(분) 숫자열(SUM 수식 합계)·업무 내용·상태. 파일명 `출장초과근로_YYYY-MM-DD.xlsx` / `출장초과근로_이름_시작일.xlsx`.
  - **인쇄 양식**: `myDocOpen`을 `docOpen(…, person)`으로 분리(사람 정보 인자화, `opt.docNo`·`opt.signDate` 지원) → `tripDocInner`가 신청서 1건을 A4 문서로(신청자·출장 내역·출장비·초과근로·**개인 사비 지출**·청구 합계·비고·관리자 확인 메모·규정). 마이페이지 '내 신청 내역'과 관리자 탭이 같은 함수를 쓴다.
  - 검증(QA 계정 3개·임시 직원 2명, 전부 삭제): 영수증 업로드 200 / 제출 시 서버 재계산(3박×20,000=60,000, 19:00~22:10 식사포함 → 130분→135분, 사비 23,000+15,000=38,000) / **B가 A 영수증 404** / B 목록 빈 배열 / 관리자 목록·영수증 200 / 직원이 관리자 액션 403 / 확인 완료 → 직원 화면 상태·메모 반영 + 양방향 알림 / 삭제 시 스토리지 객체까지 0.
- ✅ **급여 명세서 파일 업로드·본인 열람(2026-09-10 사용자 요청 — "관리자가 매월 업로드, 직원은 업로드된 파일을 확인. 워크스페이스 계산값을 보여주는 게 아님. 본인 외 외부 접근 절대 불가")**:
  - 처음엔 wm-me `salary` 액션으로 calcOne 결과를 마이페이지에 보여주는 방식으로 만들었다가(같은 날) 사용자 지시로 **계산식 화면·액션 제거**, 파일 업로드 방식으로 교체. `wmSlipHtml(r, forPdf, ym)`의 ym 인자는 남겨둠(무해).
  - **DB/스토리지**: `wm_payslips`(migrations/0007 — employee_id·ym unique, file_name·mime·size·storage_path·note·uploaded_by·uploaded_at, RLS on·정책 없음) + **비공개 버킷 `wm-payslips`**(10MB, pdf/png/jpg/webp만, 경로 `{employee_id}/{ym}/{uuid}.{ext}`). 공개 URL·서명 URL을 만들지 않고 **함수가 바이트를 중계**(anon 직접 접근 400 확인).
  - **관리자(wm-admin, admin만)**: `payslip_list {ym}` / `payslip_upload {employee_id, ym, file_name, mime, data_b64}`(base64 JSON, 8MB 제한, 직원·월당 1개 — 다시 올리면 PATCH + 이전 객체 삭제, 직원 app_user_id 있으면 `notifications`(link_menu 'my') 알림, auditLog payslip_upload/replace) / `payslip_delete {id}` / `payslip_file {id}`(관리자 확인용 중계). 화면: 근무 관리 → **'명세서 파일' 탭**(`wmSlipFilesLoad/wmSlipFileUpload/wmSlipFileDelete/wmSlipFileOpen`, 귀속 월 input[type=month] 기본 지난달, 활성 직원 전원 표에 업로드/교체·내려받기·삭제, 마이페이지 계정 미연결 표시).
  - **직원(wm-me)**: `payslip_list`(본인 것만 ym desc) / `payslip_file&id=`(employee_id=본인 조건, 타인 404). 화면: 마이페이지 → **'급여 명세서' 탭**(`myRenderPay/myPayLoad/myPayOpen`) 표(귀속 월·파일·등록일·보기/저장). 보기 = fetch → Blob URL 새 창(공유 가능한 서버 URL 없음), 저장 = a.download. 공용 헬퍼 `wmFetchBlob(fn, params, body)`·`slipOpenBlob`·`slipFmtSize`.
  - 검증(QA 계정 3개 + 임시 직원 A/B, 전부 삭제): 관리자 업로드 200 → 같은 달 재업로드 replaced·버킷 객체 1개 유지 / staff 업로드 403 / exe 400 / A 본인 목록·파일 200(바이트 일치, `Cache-Control: private, no-store`) / **B가 A 파일 404** / 무토큰 401 / 관리자 중계 200 / anon 스토리지 직접 400 / A 알림 생성.
