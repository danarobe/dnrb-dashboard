...
**진행 중 / 사용자 답 대기**
1. **반품 스캔 — 네이버페이센터 반품/교환 엑셀 샘플 대기**: 실제 열 이름을 못 봤다. 받으면 `rscanNaverUpload`의 헤더 후보(수거송장번호·상품주문번호·클레임구분·사유·택배사·요청일)를 실양식에 맞추고 실파일로 검증. 경고 코드 기본값(K·V·D / C·J·W)은 6개월 실데이터 추정 — 사용자 실사용 피드백 반영.
