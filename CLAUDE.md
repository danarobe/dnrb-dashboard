...
- netreturns: **배송완료일(품목 delivered_date) 기준** R40/R30/R34. 등급 우수<10/주의10~20/위험≥20, 배송완료 10개 미만 보류. 주문 수집은 [s−7d, e+30d] 패딩(부분배송 대비). 옵션별 접이식 상세.
- **반품 사유 모아보기(2026-08-07)**: 판매 성과 표에서 **상품명 클릭** → 모달. `returnreasons` 액션(netreturns와 동일 모수 — 같은 패딩·delivered_date·R40/R30/R34, 실측 148개 상품 732수량 완전 일치)이지만 `order_status=R40,R30,R34` 필터로 스캔량 축소(4,554→486건, **24초→5초**). **콤마 다중 상태 필터 동작함**.
...
- **순반품률 기준이 판매 성과와 일부러 다름**(사용자 결정): 여기서만 **R00(반품신청)·R10(반품접수)까지 포함**. netreturns(R30/R34/R40)는 **건드리지 않음** — 판매 성과·홈·진열은 그대로.
...
- **기준일은 오늘 −7일이 기본값.** 반품 신청은 배송완료 후 0~5일에 걸쳐 들어오고(당일 51%·3일 90%·5일 99%) 승인·수거가 더 걸려서, **오늘 날짜로 보면 실제의 절반 이하**로 나온다(실측: 오늘까지 7일 4.97% vs 확정 ~12%). 5일 이내 기준일을 고르면 빨간 경고 표시.
