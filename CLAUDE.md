...
- **⚠ 메뉴 숨김(2026-09-11 사용자 결정)**: 광고관리자 기능은 친구가 만든 별도 앱 **ad-dashboard**(공개 저장소 github.com/qkralsrb03138668/ad-dashboard, GitHub Pages `https://qkralsrb03138668.github.io/ad-dashboard/`, 친구 Supabase `pydxcqfztjogmztvayux`, 친구 서버가 Meta 데이터 5분마다 수집 — 우리 토큰·시크릿 공유 없음)로 대체. 메뉴 `menu-addash`(`adDashOpen()`, **로그인 전원**) → `auth sso_issue {aud:'ad-dashboard'}` 60초 코드 → 저쪽이 `sso_redeem`으로 전용 토큰 → 매 요청 `auth verify`. 내장 #admgr은 `menu-admgr`만 `display:none`(코드·서버 액션·ad_test_state·예산 예약 전부 유지, `#admgr` 직접 이동 가능). 되살리기 = applyRole의 한 줄. 이 앱은 친구 저장소라 우리 CLAUDE 규칙 밖 — 공동 개발하려면 친구 초대 필요(공개 저장소라 읽기는 가능).
