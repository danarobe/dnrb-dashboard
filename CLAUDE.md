...
- **광고↔상품 매칭·활성 목록 보강(2026-09-23)**: ① 카페24 상품명 끝 **' - Danarobe' 접미사**(SEO, 9/20~)로 `paKey` 핵심명이 깨져 판매 성과 'ON 광고'·에이전트가 광고 0개로 오판 → `PA_BRAND_SUFFIX`로 먼저 제거(에이전트 def.ts와 동일). ② meta-ads `activeads`: ACTIVE 필터 목록(130)에 오늘 지출 광고 83개가 빠짐(실측, 9/14~17 _test 소재) → 어제·오늘 지출 인사이트와 합집합(`recovered_from_spend`), 221개.
