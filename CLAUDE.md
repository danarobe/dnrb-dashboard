...
- 상품명에 **자체제작/made** 포함 상품(사용자 확인: 항상 포함)의 발주 시점·수량 점검. 서버 `cafe24-analytics madeavg`(admin+staff, 10분 캐시): **애널리틱스 일별 7회 + 30일 합계 1회 + 취소(C40/R40) 30일 스캔 1회** → 어제까지 3일·7일 일평균 순판매(결제−취소, 주문일 — 판매 성과와 동일 기준).
- **신상 보정(사용자 설계)**: 30일 합계 ≤ 7일 합계면 첫 판매가 7일 창 안 → 일별 시리즈에서 첫 판매일을 찾아 **분모를 실제 판매일수로**(예: 어제 첫 판매 97개 → ÷1 = 97/일. ÷7이면 13.9로 저평가 — 실사례 검증). 표에 '판매 N일' 배지+첫 판매일 표시.
...
- **재고는 셀메이트 CSV 업로드**(API 유료라 — 방식 사용자 확정): 헤더 자동 인식(상품명/현재재고·현재고/미발송·미출고 시작 헤더), EUC-KR은 기존 readFileAsText·stkNorm 재사용, 옵션 행을 상품 단위 합산. **가용재고 = 현재고 − 미발송**.
- **판정**: 소진일수 = 가용재고÷7일평균 → ≤리드타임 = 🔴주문 필요 / ≤리드+3일 = 🟠임박 / 그 외 🟢. 제작처 미지정·CSV 미업로드는 별도 상태. **권장 주문량 = ceil(7일평균 × (리드 + 커버목표) − 가용재고)**, 커버 기본 14일(localStorage `dnrb_made_cover`). 정렬 = 심각순.
- 검증: 실데이터 19상품·신상 보정 실증(÷1), 가짜 CSV 옵션 합산(90−15=75)→소진 4.7일→주문 필요·권장 290 수기 일치, MD 조회 200/지정 403, QA 태그 원복 완료.
