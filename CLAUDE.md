...
| `wm-me` | **마이페이지**(2026-08-27): me/leave_request/leave_cancel + **salary(2026-09-10, `?ym=`)**. 로그인 계정 → `wm_employees.app_user_id`로 본인 행만 조회·쓰기. me 응답엔 급여·계좌 없음, salary는 본인 명세서 1건만(employee_id 파라미터 없음·무시, 미연결 403, 미래 달 400, pin_* 제거) | 로그인 전원 |
...
- ✅ **마이페이지 급여 명세서(2026-09-10 사용자 요청 — "본인 외 외부 접근 절대 불가")**: 탭 '급여 명세서'(`myRenderPay/myPayLoad/myPayPrint`) → `wm-me?action=salary&ym=YYYY-MM`(GET). 서버가 **토큰 → app_user_id → 본인 wm_employees 1행**으로만 계산(`calcOne`을 `../wm-admin/salary.ts`에서 import, 출퇴근·휴가도 그 employee_id로만 조회), 클라이언트는 달만 보냄. 화면 요약 타일(근무시간·기본급·주휴·식대·교통비·총지급·공제·실지급) + '명세서 열기'는 근무 관리와 같은 `wmSlipHtml(r, true, ym)`(ym 인자 추가 — wmState.ym 의존 제거) → 새 창 인쇄/PDF. **저장·URL 공유 경로 없음**(새 창 내용은 브라우저 안에서만 생성). 기본 조회 월 = 지난달, 최근 13개월. 검증: QA 알바(qa_staff_tmp↔임시 직원 1012, 8/21 08:57~17:05) → 7.13h·74,900+식대 8,000=82,900 / 미연결 계정 403 / 무토큰 401 / 2026-13·미래 달 400 / `employee_id=3` 파라미터 무시(본인만) — QA 데이터 전부 삭제.
