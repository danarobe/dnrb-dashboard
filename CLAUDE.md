...
2. **진열 조기 강등 룰 조정 여부 — 사용자 결정 대기**
   - 사례 분석 완료: 나그랑 맨투맨(8/4 등록)이 신상에 없던 것 = 조기 강등 정상 동작(경과 7.1일·조회 1,057회·블록 점수 하위 3/12). 멜리 레이어드 나시는 판매 0이어도 조회 149회<210이라 '판단 유보'로 신상 유지 — 둘 다 설계 의도대로.
   - 열린 제안: ⓐ 조기 강등 기본값(dcfg-demote 210) 조정 — 상품팀 협의 권고 ⓑ "N일 지나도 조회 X회 미만이면 강등" 같은 저노출 신상 처리 규칙 추가 여부 — 현재는 조회 못 받는 신상이 21일 내내 신상 블록에 남음.
